...
const size_t DEFAULT_LIST_SIZE = 30;
...
int test2( )
{
    ...
    cout << DEFAULT_LIST_SIZE*10 << " at the sequence's end." << endl;
    for (i = 4; i <= DEFAULT_LIST_SIZE; i++)
        test.attach(i*10);

    // Test that the sequence is correctly filled.
    cout << "Now I will test that the sequence has 10, 20, 30, ...";
    cout << DEFAULT_LIST_SIZE*10 << "." << endl;
    test.start( );
...
int test4( )
{
    ...
    test.resize(2*DEFAULT_LIST_SIZE);
    test.attach(0);
    ...
    for (i = 1; i < 2*DEFAULT_LIST_SIZE; i++)
        test.attach(i);
    test.start( );
    memcpy(newbytes, (char *) &test, sizeof(sequence));
     
    for (i = 0; i < 2*DEFAULT_LIST_SIZE; i++)
    {
        if (test.current( ) != i)
        {
    ...
    cout << DEFAULT_LIST_SIZE*2 << " items." << endl;
    memcpy(bytes, (char *) &test, sizeof(sequence));
    test.resize(1);
    mismatches = 0;
    for (i = 0; i < sizeof(sequence); i++)
...
int test5( )
{
    ...
    double items[2*DEFAULT_LIST_SIZE];
    size_t i;
    ...
    for (i=2; i <= 2*DEFAULT_LIST_SIZE; i++)
        original.attach(i);
    ...
    if (!correct
        (copy2, 2*DEFAULT_LIST_SIZE, 2*DEFAULT_LIST_SIZE-1, items)
        )
        return 0;
    ...
    original.remove_current( ); // Removes 2 from the original, but not the copy.
    if (!correct
        (copy3, 2*DEFAULT_LIST_SIZE, DEFAULT_LIST_SIZE, items)
        )
        return 0;
    ...
    original.remove_current( ); // Removes 2 from the original, but not the copy.
    if (!correct
        (copy4, 2*DEFAULT_LIST_SIZE, 0, items)
        )
        return 0;
    ...
    original.remove_current( ); // Removes 2 from the original, but not the copy.
    if (!correct
        (copy5, 2*DEFAULT_LIST_SIZE, 2*DEFAULT_LIST_SIZE, items)
        )
        return 0;
...
int test6( )
{
    ...
    original = original;
    if (!correct
        (original, 2*DEFAULT_LIST_SIZE, 1, items)
        )
        return 0;
...
int test7( )
{
    ...
    for (i = 1; i <= 2*DEFAULT_LIST_SIZE; i++)
        testa.attach(i);
    if (!correct
        (testa, 2*DEFAULT_LIST_SIZE, 2*DEFAULT_LIST_SIZE-1, items)
        )
        return 0;

    cout << "Testing to see that insert works correctly when the\n";
    cout << "current capacity is exceeded." << endl;
    for (i = 2*DEFAULT_LIST_SIZE; i >= 1; i--)
        testi.insert(i);
    if (!correct
        (testi, 2*DEFAULT_LIST_SIZE, 0, items)
        )
        return 0;
